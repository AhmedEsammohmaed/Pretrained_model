...
st.caption(
    "Automated extraction of structural hierarchies, wall thicknesses, and RF"
    " attenuation using Mask2Former."
)
...
# -----------------------------------------------------------------------------
# 2. Optimized Model Caching (CPU Low-RAM Safe)
# -----------------------------------------------------------------------------
...
def load_segmentation_model():
    ...
    # Low CPU memory usage prevents 1GB OOM crashes on Streamlit Cloud
...
def process_and_draw_floorplan(
    pil_image, scale_cm_per_px, confidence_threshold
):
    ...
    for info in segments_info:
        ...
        if "wall" in label_name:
            ...
            building_structure["Building"]["Doors"].append(
                {"type": "Door", "bbox": [int(x) for x in bbox]}
            )
            label_text = "Door"
            ...
            building_structure["Building"]["Windows"].append(
                {"type": "Window", "bbox": [int(x) for x in bbox]}
            )
            label_text = "Window"
...
with st.sidebar:
    ...
    scale_cm_per_px = st.slider(
        "Scale: Pixel to CM Ratio", 0.5, 10.0, 2.5, 0.1
    )
    confidence_threshold = st.slider(
        "Detection Threshold", 0.1, 0.9, 0.35, 0.05
    )

uploaded_file = st.file_uploader(
    "Upload Architectural Floor Plan (PDF, PNG, JPG, WEBP):",
...
if uploaded_file is not None:
    ...
    if st.button("🚀 Analyze Floor Plan", type="primary"):
        with st.spinner(
            "Processing segmentation & calculating RF parameters..."
        ):
            json_output, annotated_image = process_and_draw_floorplan(
                raw_image, scale_cm_per_px, confidence_threshold
            )
        ...
        st.download_button(
            ...
            file_name=(
                f"{os.path.splitext(uploaded_file.name)[0]}_structure.json"
            ),
